...
// x(t) = A cos[2 pi f0 (t-t0) + phi] to an existing Signal object.   *
// The math expression is also set accordingly.                       *
//*********************************************************************
void Signal::makeSinusoid(double A, double f0, double phi)
{
	...
	for (unsigned int n=0; n<nSamp; n++)	// Loop over elements
		...
		s[n] = A * cos(2*PI*f0 * (t[n] - t0) + phi);

	// Assemble math expression for sinusoid
	expression = textNum(A) + " cos( ";
	expression += textNum(2*PI*f0);
	// "(t +/- t0) " unless t0 = 0, then "t "
	if (t0 != 0)
	{
		expression += " (t ";
		expression += (t0 < 0) ? "+ " : "- ";
		expression += textNum(fabs(t0)) + ") ";
	}
	else
		expression += " t ";
	// append phase, with appropriate sign
	if (phi != 0)
	{
		expression += (phi > 0) ? "+ " : "- ";
		expression += textNum(fabs(phi));
	}
	expression += " )";
}
...
//*********************************************************************
// makeRamp writes samples of the linear signal m(t - t0) + b to an   *
// existing Signal object, where m is the slope, to is the initial    *
// time, and b is the value at t = t0.  The math expression is also   *
// set accordingly.                                                   *
//*********************************************************************
void Signal::makeRamp(double slope, double intercept)
{
	for (unsigned int n=0; n<nSamp; n++)		// Loop over elements
		s[n] = (t[n] - t0)*slope + intercept;	// s(t) = m(t-t0) + b

	// Assemble math expression for ramp signal
	expression = textNum(slope) + " ";
	if (t0 < 0)
		expression += "(t + " + textNum(-t0) + ") ";
	else if (t0 > 0)
		expression += "(t - " + textNum(t0) + ") ";
		...
		expression += "t ";
	expression += (intercept >=0) ? "+ " : "- ";
	expression += textNum(abs(intercept));
}
...
Signal Signal::operator + (const Signal &sigIn)
{
	...
	if ((nSamp == sigIn.nSamp) && (fs == sigIn.fs) && (t0 == sigIn.t0))
	{
		...
		// temp storage for expression to append
		string expr_temp = sigIn.expression;
		// if the first character of the expression to append is "-"
		if (expr_temp.compare(0,1,"-") == 0)
		{
			// first append a minus sign w. spaces
			sigOut.expression = expression + " - ";
			// then append the expression, except for 1st char
			sigOut.expression += expr_temp.erase(0,1);
		}
		// otherwise, the expression to append is not negative
		else
		{
			// first append a plus tisng with spaces
			sigOut.expression = expression + " + ";
			// then append the expression
			sigOut.expression += expr_temp;
		}
