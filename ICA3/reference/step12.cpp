...
#include <iostream>
// iomanip library needed for setpercision() and fixed manipulators
#include <iomanip>
...
int main()
{
	...
	if (v_or_c == "v" || v_or_c == "V")
	{
		...
		//Compute the inner product of the vectors using both
		//polar and Cartesian coordinates
		double dot_prod1 = R1 * R2 * cos(theta1 - theta2);
		//double dot_prod1 = R1 * R2 * cos(theta1*pi/180 - theta2*pi/180);
		double dot_prod2 = x1*x2 + y1*y2;
		
		//Report the two inner product values to the user
		cout << "The inner product of v1 and v2 using polar ";
		cout << "coordinates is " << dot_prod1 << endl;
		cout << "The inner product of v1 and v2 using Cartesian ";
		cout << "coordinates is " << dot_prod2 << "\n\n";
	}
	else
	{
		...
		//Compute the product of the complex numbers using both
		//polar and rectangular coordinates
		double R_prod = R1*R2, theta_prod = theta1+theta2;
		theta_prod -= 2*pi*( theta_prod > pi );
		theta_prod += 2*pi*( theta_prod <- pi );
		double x_prod = x1*x2 - y1*y2, y_prod = x1*y2 + y1*x2;
		
		//Report both products to the user
		cout << "Product, polar:\t\t z1 * z2 = " << R_prod << " exp("; 
		cout << (theta_prod>= 0 ? "j " : "-j ") << abs(theta_prod) << ")\n";
		cout << "Product, Cartesian:\t z1 * z2 = " << x_prod; 
		cout << (y_prod >= 0 ? " +" : " -") << " j " << abs(y_prod) << "\n";
		
		//Convert polar to Catesian coordinates, and report
		double x_prod_convert = R_prod * cos( theta_prod );
		double y_prod_convert = R_prod * sin( theta_prod );
		cout << "Polar, converted to Cartesian: z1 * z2 = " << x_prod_convert; 
		cout << (y_prod_convert >= 0 ? " +" : " -") << " j " << abs(y_prod_convert) << "\n";
	}
